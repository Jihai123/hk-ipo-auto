...
function extractNetProfit(text, options = {}) {
  ...
  function analyzeRawValue(rawValue) {
    const raw = String(rawValue || '').trim();
    const compact = raw.replace(/\s+/g, '');
    const digitsOnly = compact.replace(/[^\d]/g, '');
    const commaParts = compact.split(',').filter(Boolean);
    const rejectFlags = [];

    if (!digitsOnly) {
      rejectFlags.push('invalid_number');
    }

    if (commaParts.length >= 4) {
      const malformedGroup = commaParts
        .slice(1)
        .some(part => !/^\d{3}(?:\.\d+)?$/.test(part));
      if (malformedGroup || commaParts.length > 5) {
        rejectFlags.push('merged_table_numbers');
      }
    }

    if (digitsOnly.length > 12) {
      rejectFlags.push('value_too_long');
    }

    if (/^\d{1,2}$/.test(digitsOnly)) {
      rejectFlags.push('tiny_scalar_value');
    }

    return { raw, digitsOnly, rejectFlags };
  }
  ...
  function normalizeProfitCandidate(candidate) {
    const rawAnalysis = analyzeRawValue(candidate.rawValue);
    ...
    if (!Number.isFinite(numericValue)) {
      return {
        ...
        rawAnalysis,
        rejectFlags: [...candidate.rejectFlags, ...rawAnalysis.rejectFlags],
        rejectFlag: true,
      };
    }
    ...
    return {
      ...
      rawAnalysis,
      rejectFlags: [...candidate.rejectFlags, ...rawAnalysis.rejectFlags],
      rejectFlag: candidate.rejectFlag || rawAnalysis.rejectFlags.length > 0,
    };
  }
  ...
  function scoreProfitCandidate(candidate) {
    ...
    if (candidate.rejectFlags.includes('merged_table_numbers')) score -= 150;
    if (candidate.rejectFlags.includes('value_too_long')) score -= 120;
    if (candidate.rejectFlags.includes('tiny_scalar_value')) score -= 80;
    if (/每股|eps|earningspershare/i.test(candidate.matchedText)) {
      candidate.rejectFlags.push('eps_like');
      candidate.rejectFlag = true;
    }
    if (candidate.periodType === 'unknown' && candidate.sourceLevel === 'table') {
      candidate.rejectFlags.push('year_period_unbound');
      candidate.rejectFlag = true;
      score -= 60;
    }
    if (candidate.netProfitHKD !== null && Math.abs(candidate.netProfitHKD) > 1e12) {
      candidate.rejectFlags.push('profit_outlier');
      candidate.rejectFlag = true;
      score -= 120;
    }
  ...
  function chooseBestProfitCandidate(candidates) {
    ...
    const usableCandidates = sorted.filter(c => !c.rejectFlag);
    const validAnnual = usableCandidates.filter(c => c.periodType === 'annual');
    const valid = validAnnual.length ? validAnnual : usableCandidates;
  ...
  if (best.topGapTooSmall || best.periodType !== 'annual' || best.rejectFlag) {
    console.log(`[netProfit] 候选不满足正式PE使用条件: label=${best.label}, reason=${best.reason}`);
    return {
      hkdAmount: null,
      netProfitHKD: null,
      source: best.source,
      ...
      confidence: 'low',
      reason: best.reason || 'insufficient_data',
      score: best.score,
...
function scorePE(offerPriceMid, totalShares, netProfitHKD, peerMedianPE, meta = {}) {
  ...
  if (computedPE <= 0 || computedPE < 0.5 || computedPE > 300 || netProfitHKD < 1e6) {
    return {
      score: 0,
      reason: 'PE：疑似异常值',
