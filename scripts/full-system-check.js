 ...
 *   BASE_URL=https://your-domain.com ENTRY_PATH=/hk/ API_BASE=/api node scripts/full-system-check.js
 */
...
const ENTRY_PATH = argMap.entry || process.env.ENTRY_PATH || '/hk/';
const API_BASE = argMap.api_base || process.env.API_BASE || '/api';
...
function getAltApiBase(entryPath) {
  const clean = String(entryPath || '/hk/').replace(/\/+$/, '');
...
function gradeReport(report) {
  ...
  const routeProbe = report.backend.route_probe || {};
  if (routeProbe.root_status >= 400 && routeProbe.alt_status < 400) issues.push('ROUTE_PREFIX_MISMATCH');
...
async function main() {
  ...
  const apiTargets = [
    `${API_BASE}/dashboard?sort=score`,
    `${API_BASE}/score/${SCORE_CODE}`,
    `${API_BASE}/ipo/top?limit=3`,
    `${API_BASE}/ipo/current`,
    `${API_BASE}/market/stats`,
  ];

  const apiPayload = {};
  for (const p of apiTargets) {
    try {
      const r = await httpGet(joinUrl(BASE_URL, p));
  ...
  // 1.1) 部署路径一致性检查（用于识别 Nginx / 前端路径与 API 路径不一致）
  try {
    const altApiBase = getAltApiBase(ENTRY_PATH);
    const rootProbe = await httpGet(joinUrl(BASE_URL, `${API_BASE}/dashboard?sort=score`));
    const altProbe = await httpGet(joinUrl(BASE_URL, `${altApiBase}/dashboard?sort=score`));
    report.backend.route_probe = {
      api_base: API_BASE,
      entry_path: ENTRY_PATH,
      root_status: rootProbe.status,
      alt_api_base: altApiBase,
      alt_status: altProbe.status,
      hint: (rootProbe.status >= 400 && altProbe.status < 400)
        ? '可能存在前端/API 路由前缀不一致，请检查 Nginx 对 /api 的转发'
        : '',
    };
  } catch (error) {
    report.backend.route_probe = { error: error.message };
  }

  // 2) frontend page checks
  try {
    const page = await httpGet(joinUrl(BASE_URL, ENTRY_PATH));
  ...
  const dashboard = apiPayload[`${API_BASE}/dashboard?sort=score`] || {};
  ...
  report.data_quality = {
    ...
    usable_top3_count: top3.filter((x) => !normalizeMissing(x.name) && !normalizeMissing(x.code)).length,
    usable_leaderboard_count: leaderboard.filter((x) => !normalizeMissing(x.name) && !normalizeMissing(x.code)).length,
...
function finalize(report) {
  ...
  if (report.backend.route_probe) {
    const rp = report.backend.route_probe;
    console.log(`路由探针: api_base=${rp.api_base || ''} root_status=${rp.root_status || ''} alt_base=${rp.alt_api_base || ''} alt_status=${rp.alt_status || ''} ${rp.hint || ''}`);
  }
  ...
  console.log(`可用 Top3 条数: ${report.data_quality.usable_top3_count}`);
  console.log(`可用 榜单条数: ${report.data_quality.usable_leaderboard_count}`);
